...
<head>
    ...
    <script>
        function confirmdelete(bookmark_id) {
            var result = confirom("Are you sure you want to delete the reserve ID" + bookmark_id + "?");
            if (result) {
                window.location.href = "delete_reserve.php?bookmarkid=" + bookmark_id;
            }
        }
        ...
        $(document).ready(function() {
            ...
            $("#dateFilter, #marksearch, #active").on("input", function() {
                var selectedDate = $("#dateFilter").val();
                var filter = $("#filter").val();
                var search = $("#marksearch").val();
                $("#active").val($("#active").is(":checked") ? 1 : 0);
...
<body data-needs-fetch-rooms="true">
    ...
    <dactive: active,class="row" id="markList">
